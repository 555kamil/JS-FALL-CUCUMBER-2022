...
Then(/^I verify error is displayed when user submits the empty feedback form$/, async function() {
    expect(await homePage.isEmptyFeedbackErrorDisplayed(), 'Error is NOT displayed when user submits the empty feedback form').to.be.true;
    await browser.pause(2000);
});

Then(/^I verify star boxes section is in a red dotted box$/, async function() {
    expect(await homePage.isRedDotsBoxDisplayed(), 'Star boxes section is NOT in a red dotted box').to.be.true;
    await browser.pause(2000);
});